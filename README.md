...
- **PII 值级掩码采样（v0.9.43+，默认关闭）**：`PII_VALUE_SAMPLE_ENABLED=1` 时 PII 分布 hover 从仅计数扩展为 `计数 + TopN 掩码值 x 次数`（如 `phone: 123 (138****8000 x5, 139****1111 x2)`），掩码在 `_pii.py` 命中时当场生成（明文不出作用域，`masked_sample+hash(16hex)` 进入 `pii_value_samples` Top5 聚合展示前3），默认仅内存 `recent_events` 10k 环现场聚合（`range=1h` 精确 `is_precise` 同 `ring_coverage`，`PERSIST=0` 时 `24h/7d` 返回 `{}` 且 `is_precise=false` 不读盘，热切换原子），`PII_VALUE_SAMPLE_PERSIST=1` 时落盘 `pii_value_agg(day,upstream,kind,hash)` 按日覆盖式 UPSERT≤40行/日、7天滚动 `0600`（含 `-wal/-shm`）；前端 Chart.js `afterBody` + SVG `<title>` 多行 `kind: count\nmasked x count` 经 `textContent/title` 防 XSS，`truncated` 时尾加 `…长尾仅计 pii_by_type`，`PERSIST=0 && range!=1h` 灰显`仅1h精确`；`hash` 截断小空间可枚举仅去重，文档声明侧信道（phone 7位暴露1万匿名集、email首字符泄漏）可选 HMAC/每日盐。
...
| `PII_VALUE_SAMPLE_ENABLED` | PII 值级掩码采样总开关：`1` 时仪表盘 PII 分布 hover 展示 TopN 掩码值及次数（聚合 Top5 展示前3，掩码形如 `138****8000`/`a***@b.com`/`**** **** **** 6789`/`192.168.**.**`/`前4****后4`，长度上限64，hash 为 `sha256(明文)[:16]` 16hex 仅去重），仅对话端点 `is_chat_tail` 触发，非对话不采样；默认仅计数 | `0`（默认关闭，仅计数） |
