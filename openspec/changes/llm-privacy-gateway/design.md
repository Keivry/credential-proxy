...
- 判定用**字符串前缀匹配**（`startswith("10.")`、`startswith("192.168.")` 等），不构造 `ipaddress` 对象——实测 ipaddress 逐候选过滤 ~12μs/候选、1MB body 9021 候选 +108ms，前缀匹配近似 0 开销。IPv6 因 `::` 简写形式正则难覆盖，用**正则粗筛（含 `:` 的 token）→ 字符串/`ipaddress` 精确解析**两级判定。
...
- **ReDoS 防护**：用户自定义正则不可信，必须带**线程超时守卫**（`asyncio.to_thread` + `asyncio.wait_for`，预算 100ms）——实测 `^(a+)+$` 匹配 30 字符耗时 57 秒，恶意/失误正则可卡死事件循环；启动时编译 + 自检样本跑一遍，超时/异常即拒绝加载该规则并告警
- **`\b` 边界禁止**：`\b` 是 ASCII 词边界，中文汉字非 `\w`，实测 `'联系13812345678处理'` 下 `\b1[3-9]\d{9}\b` **零命中**（中文紧贴漏检）——内置与自定义正则一律改用 lookaround（`(?<!\d)...(?!\d)`）
- 正则用**命名捕获组**（`(?P<user1>...)`），联合正则合并后按 `lastgroup` 区分类型

**字典型 recognizer（可配置）**：支持配置**敏感名称名单**（人名、工号、内部主机名、内部域名等）做精确匹配替换。中文人名**无格式规则**（非模式而是实体，无分隔符/校验位/上下文），正则覆盖不了，名单匹配是零依赖可行方案。**性能（硬性）**：字典型**独立扫描，不并入联合正则**——实测 5000 名字典并入联合正则后单 1KB chunk 扫描 124µs → 13.8ms（**慢 111 倍**，alternation 分支爆炸）；独立扫描 5000 名仅 190µs。字典按长度降序排序 + `re.escape` + 单条编译（5000 名编译 61ms，**编译缓存**避免热路径重编译）。配置变更时显式重编译。
...
- **性能锚点（实测更新）**：1MB body 联合正则 ~90ms（含分类）、纯文本粗筛 ~0.8ms、单 1KB chunk ~124µs、正常 <100KB 请求 <10ms——design 原声称「μs 级」修正为「百 μs 级/1KB、十 ms 级/1MB」
...
- **实现**：`_split_safe_hold` 增强——在现有 `rfind('__')` token 分割基础上，**优先在最后一个标点边界（`，。！？；：、,.!?;:()[]{}<>"'` 与换行/空格）处切分**；若最后标点距尾部超过 `PII_HOLD_MAX`（默认 64 字符，覆盖最长 token `__PII_<seq>_<rand4>__` 32 字符 × 2 安全余量），才回落尾部 64 字符字节持有；**标点边界不破坏 token 完整性**（token 形态 `__PII_` 内无标点，`rfind('__')` 优先）
- **缓冲大小**：字段级累积 + 标点边界下，缓冲持有量 ≈ 最后一句未完成文本（通常 <100 字符），远小于固定 1KB-4KB 字节缓冲；内存实测 4KB × 50 并发 = 200KB，标点边界下进一步降低
...
- **残缺 token 处理**（复用 `_split_safe_hold` + `_PARTIAL_TOKEN_RE` 语义）：完整 token 保留（等还原）、残缺前缀（`__PII_0001_`）剥离防泄漏、残缺后缀（`a3f2__`）暂存等前半拼齐、流结束未拼齐丢弃；**PII 版残缺清理正则独立于凭据版**（不得 import 凭据正则）
