...
- **主动 PII 检测与脱敏（双向）**：在现有凭据 token 替换基础上，新增 PII 检测层——请求 body 检测身份证号、手机号、邮箱、银行卡、IP、常见 API key 等模式，值注册进 token 映射后替换为占位符（**值注册校验：拒绝 token 形态值及包含 token 形态子串的值**，防与占位符自身混淆）；响应侧同样检测，防止 LLM 回显/复述用户 PII，并还原请求侧脱敏产生的占位符。**保留地址（私网/环回/链路本地/组播/文档前缀等）豁免脱敏**（环境参数而非隐私）；支持**可配置自定义正则**（工号、项目代号、内部域名等）与**敏感名称名单**（人名、工号、内部主机名等字典型匹配）参与检测
- **PII token 请求级作用域**：检测出的 PII 值使用请求级 token 映射（请求生命周期内有效、请求结束即清理、不写入全局映射；含随机段防枚举），避免内存膨胀与跨请求串扰
- **流式标点边界缓冲**：流式响应按语义边界（标点/换行）切分缓冲、尾部持有（默认 64 字符），候选值感知切分防止切断 IP/邮箱等含标点值，UTF-8 按字符处理，残缺占位符不泄漏
...
- **新增文件**：`_pii.py`（PII 检测/替换）、`_audit.py`（输出审计策略引擎，审计日志 JSON Lines 追加写 `DATA_DIR/audit.log`）、`examples/audit-policy.yaml`（AUDIT_POLICY_FILE 示例策略文件，含「检测器」维度示例与防护边界注释）
