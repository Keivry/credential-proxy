...
系统 SHALL 在 LLM 请求转发前检测正文中的 PII（至少包含：中国大陆身份证号、手机号、电子邮箱、银行卡号、IPv4 地址、常见 API key/密钥格式（如 `sk-`、`ghp_` 前缀格式）），并将检测到的值替换为占位符；系统 SHALL 在 LLM 响应中检测并还原请求阶段产生的占位符，还原后的文本与原文一致。流式响应中跨分片切断的 PII（占位符残缺或明文被分片）SHALL 由分片累积机制处理，不得泄漏残缺片段。
