...
- [x] 1.2 改造 `PiiMixin` 为全局单例：`_init_pii` 创建 `self._global_pii_scope` 单例，`_pii_request_scope()` 返回单例复用、`_pii_cleanup()` 不再 `clear()`，`_pii_detector.request_tokens` 始终指向单例
...
- [x] 1.3 同步 `proxy.py` 与 `_pii.py` 的初始化与清理路径，移除每请求 `new/clear` 的残留引用
  - 验收：`grep -rn "RequestScopedTokens(" --include="*.py" _pii.py | wc -l` 为 1（初始化，`_token.py:187` 兼容别名 `RequestScopedTokens = GlobalPiiTokens` 不计，`_pii.py` 中 `from _token import ...RequestScopedTokens` 为导入非构造），`grep -n "_pii_cleanup.*clear"` 不再出现
...
- [x] 2.1 将 `LLM` 代理每请求可变状态从 `self` 迁至 `ContextVar`/局部：至少 `pii_scope / audit_arg_accum / audit_hold_active / audit_hold_buf / audit_hold_bytes / last_responses_tool_name / last_anthropic_tool_name / tool_calls_buf / tool_calls_pending_events / tool_calls_audited / tool_calls_blocked / sse_event_count / fast_sse_event_count / bytes_written / audit_block_injected / is_responses_stream / is_anthropic_stream / content_buf / reasoning_buf / arg_buf` 等全量
  - 验收：`grep -rn "self\._pii_\|self\._audit_\|self\._last_" _llm.py _pii.py _audit.py` 仅在 `ContextVar` property 定义处命中；`grep -rn "self\.tool_calls_\|self\.sse_event_\|self\.bytes_written" _llm.py` 为 0 且 handler 局部 `grep -n "^\s*tool_calls_\|bytes_written"` 命中；重链路 `audit_enabled()/pii_active()` 仍走 heavy 但并发不再串扰
...
- [x] 2.2 定义 `contextvars.ContextVar` 并在 `handler` 入口 `set`、 `finally` 中 `reset`，确保 `PiiDetector` 与 `AuditMixin` 回调通过 `ContextVar.get()` 读取而非 `self` 直读
  - 验收：并发 2 请求（不同 `PII` + 一请求进 `audit_hold`）回归通过，互不覆盖与串扰
...
- [x] 4.1 补 PII 稳定性与并发单测 + 空流单测，更新 `audit_approve_stream_test.py` 的空流断言为 `bytes_written` 语义
  - 验收：`pytest -q` 全绿（含新增 3+ 用例），`ruff check .` 与 `ruff format --check .` 全绿

- [x] 4.2 本地联调：`PII_REDACTION_ENABLED=1` 下连续同请求脱敏后一致；`v1/responses` 空流与 `hold` 悬挂场景复现不复现
  - 验收：`pytest -k test_pii_stability -v` + `pytest -k test_empty_stream_injection -v` 自动化通过；`curl` 经 `credential-proxy` 到上游的脱敏后 `body` 两次 `diff` 为 0 为手动补充（需捕获上游请求体，非 `curl` 客户端直接可观测）

## 5. 版本与发布

- [x] 5.1  bump `pyproject.toml` / `README` 版本至 `v0.9.6`，更新 `CHANGELOG` 与 `openspec` 产物的归档前置
  - 验收：`openspec validate llm-pii-cache-concurrency --strict` 通过，`tasks.md` 全勾后 `openspec status` 显示 `All artifacts complete`
