...
系统 SHALL 在 PII 命中时产出值级掩码采样：在 `_pii.py` 命中回调内（明文作用域内，仅 `PII_VALUE_SAMPLE_ENABLED=1` 且 `is_chat_tail` 为真时）生成 `masked_sample`（phone→`138****8000`；email→`a***@b.com`；bank_card→`**** **** **** 6789` 仅后4；ipv4→`192.168.**.**`；ipv6/api_key→`前4****后4`；其他→`前3****后3` 且 `<6` 时 `前1****后1`，`masked_sample` 长度上限 64）与 `value_hash=sha256(明文).hexdigest()[:16]`（变量名 `value_hash`，落盘/API 键为 `hash`），通过 `ContextVar _req_pii_var.pii_value_samples: dict[str, dict[str, dict]]`（`masked->{count, hash}`）透传到 `_metrics.py` 按 `kind` 聚合 TopN（聚合 5，展示取前 3），kind 总数受 `sanitize_kind` 白名单约束（≤8 kind，`incr_event` 消费后 `ctx['pii_value_samples'].clear()` 并 `handler finally reset`），明文不出 `_pii.py` 作用域，仅 `masked_sample+hash` 进入聚合与展示（`hash` 小空间仍可枚举，文档声明侧信道）。
...
- **THEN** 响应中不含 PII 明文，仅 `kind`、`count`、`masked_sample`、`hash` 与 `[REDACTED:<kind>]` 形态；`hash` 仅用于去重，不可还原明文。

### Requirement: 值级采样查询与持久化

`GET /_admin/metrics?range=1h|24h|7d|30d` SHALL 在 `PII_VALUE_SAMPLE_ENABLED=1` 时返回 `pii_value_samples`（聚合 Top5，展示前3）与 `pii_value_samples_is_precise: bool` 及 `pii_value_samples_truncated`（仅超 Top5 时 `true`）；`range=1h` 时从内存 `recent_events` 现场聚合（精确，`is_precise` 与 `ring_coverage` 一致），`range=24h|7d|30d` 时若 `PII_VALUE_SAMPLE_PERSIST=1` 则从 `pii_value_agg` 日级归并否则返回 `{}` 且 `pii_value_samples_is_precise=false` 不读盘；`_handle_sse` 响应头含 `Cache-Control: no-store`。可选持久化表 SHALL 为 `pii_value_agg(day TEXT, upstream TEXT, kind TEXT, hash TEXT, masked_sample TEXT, count INT, PRIMARY KEY(day, upstream, kind, hash))` 按日覆盖式 UPSERT，7 天滚动 `DELETE WHERE day < ?`，文件与 `-wal/-shm` 均 `0600`。
