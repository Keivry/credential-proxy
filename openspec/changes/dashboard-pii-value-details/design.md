...
**决定：** 在 `_pii.py` 的联合正则命中回调（`_id_card_ok/_luhn_ok/_is_reserved_ip` 校验后）当场生成 `masked_sample`（phone: `138****8000`；email: `a***@b.com`；bank_card: `**** **** **** 6789`（仅后4，BIN 不保留）；ipv4: `192.168.**.**`；ipv6/api_key: `前4****后4`；其他: `前3****后3` 且 `<6` 时 `前1****后1`）与 `value_hash=sha256(明文).hexdigest()[:16]`（变量名 `value_hash`，落盘/API 键为 `hash`）一并通过 `ContextVar _req_pii_var` 的 `pii_value_samples: dict[str, dict[str, dict]]`（`masked -> {count, hash}`）传给 `_metrics.py`，明文不跨模块。仅当 `PII_VALUE_SAMPLE_ENABLED=1` 且为对话请求（`is_chat_tail` 为真）时才产采样；调用方保证非对话不调 `scan()`，`incr_event` 锁外拷贝 `delta` 后立即 `ctx['pii_value_samples'].clear()` 并 `handler finally` 统一 `reset_req_pii_ctx()` 防跨请求叠加。`masked_sample` 长度上限 64，超长截断。
...
**决定：** `_metrics.py` 的 `_DailyAgg/_HourlyAgg` 各加 `pii_value_samples`（`dict[str, dict[str, dict]]` 结构 `masked->{count, hash}`），`incr_event` 内按 `delta_pii_value_samples` 合并 `count+=1`，kind 内按 `count` 降序截 Top5（聚合 Top5），kind 总数沿用 `sanitize_kind` 白名单（≤8 kind），最差 8*5=40 条/请求；`recent_events` 入队时精简为 `{masked: count}` 不存 hash。`incr_event` 锁内仅排序截断不 `await`，`query_range('1h')` 在锁外快照后聚合。
...
**决定：** `admin.html:renderBar('pii')` 的 Chart.js `tooltip.callbacks.label` 保持 `kind: count`（`Number(c.raw).toLocaleString()` 精确），新增 `afterBody` 返回 `Top 3: 138****8000 x5, ...`（取聚合 Top5 的前 3，`count` 均 `toLocaleString()` 精确非 `fmtNum` 缩写，`textContent` 文本通道不用 `innerHTML`）；SVG 降级 `rect <title>` 改为 `kind: count\n138****8000 x5\n...` 多行（`rect<title>` 与 `text title` 分置不同元素，禁止 `g` 级 `title`）；无采样（`ENABLED=0` 或空）或 `pii_value_samples_is_precise=false` 时仅计数不显 TopN/`…`；`SSE` 的 `metrics` 快照响应头补 `Cache-Control: no-store`。
...
**决定：** `GET /_admin/metrics?range=1h|24h|7d|30d` 新增 `pii_value_samples` 字段；`1h` 从 `recent_events` 现场 TopN 精确聚合，`24h/7d/30d` 若 `PII_VALUE_SAMPLE_PERSIST=1` 则从 `pii_value_agg` 归并否则返回 `{}` 并在响应加 `pii_value_samples_is_precise: bool` 标注。

**理由：** 值级是“当前窗口分布”而非时序，`series` 的桶序列无需值级；保持 `series` 轻量。

## Risks / Trade-offs

- [掩码仍可侧信道推断] → 聚合 Top5/展示 Top3，不展示 `hash` 全值；开关默认关；`hash` 截断小空间仍可枚举，文档声明侧信道（phone 7 位暴露 1 万匿名集、email 首字符泄漏）可选 HMAC/每日盐。
- [高基数截断导致长尾不可见] → 聚合 Top5（展示前3）截断文档化，API 顶层 `pii_value_samples_truncated: {kind: bool}` 且 `truncated:true` 仅当 `ENABLED=1 && 非空 && 超 Top5` 时置位；长尾仍在 `pii_by_type` 总数中，前端显示 `…长尾仅计 pii_by_type`。
- [内存环 1h 窗口外不可见] → `24h/7d` 需 `PERSIST=1` 才有值级，否则 hover 仅计数；前端据 `is_precise` 灰显提示。
- [掩码碰撞（不同明文同掩码）] → 以 `hash` 去重、`masked_sample` 展示，碰撞时计数合并但 hash 不同则分桶（按 `masked_sample` 聚合不按 hash，碰撞低概率可接受）。
- [SQLite 写入放大] → 每 5min 快照 UPSERT 仅日级 40 行级别，`queue.Queue(maxsize=512)` 单写者串行（`QUEUE_MAXSIZE=512`，与 `dropped_snapshots` 一致），压力可忽略；`os.umask(0o077)` 紧邻建库后立即 `_chmod_0600(day, -wal, -shm)`。
- [XSS via masked_sample] → 掩码仅含 `* . @` 等安全字符，仍经 `textContent`/`title` 文本通道，不进 `innerHTML`。
...
- `bank_card` 已定仅后4 `**** **** **** 6789`（BIN 不保留，合规）；`email` 形态 `a***@b.com` 后续可改为 `***@***.com` 以消歧。
