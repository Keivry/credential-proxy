...
系统 SHALL 在 PII 检测命中、凭据注册/还原、LRU 淘汰路径采集计数：PII 按 `kind`（内置 phone/id_card/email/bank_card/ipv4/ipv6/api_key + 自定义正则名 + 字典类型）分两类计数——`pii_detected_total{kind}`（`PiiDetector.scan` 检测到即 +1，scan 为 async 且返回 `[(type,value)]` 列表，命中钩子挂在 scan 命中处）与 `pii_cache_hit/miss`（`GlobalPiiTokens.register` 命中已注册复用即 `hit`、新建即 `miss`；**仅请求侧合法值计数——`not value` 空值与 `PII_TOKEN_STR_RE.fullmatch(value)`/`TOKEN_PREFIX in value` 非法 token 形态的提前返回分支不计数**（`_token.py:268-280` 空值/非法提前 return/raise）；**响应侧 `resp_p2t` 还原不参与 `pii_cache_*` 计数**）及 `pii_lru_evictions`（`while len(table_p2t)>PII_MAX_ENTRIES: popitem(last=False)` 淘汰时按实际弹出条数批量 `+=n`，两表 `pii_p2t/resp_p2t` 各自触发均累加，暴露到 `/_admin/metrics` 与 `health` 含 `first_dropped_ts/last_dropped_ts` + `dropped_snapshots`），凭据按 `cred_hit/cred_miss`（`_register_secret` 新建与 `_redact` 快照命中均计 `hit`，按请求 `out!=in` 计 1 而非替换次数）与 `cred_lru_evictions`（`len(pwd_to_token)>=MAX_TOKEN_ENTRIES` 分支内 `next(iter(pwd_to_token))` 取最老并 `pop(oldest)` 时 +1，文案旧称 `popitem(last=False)` 已对齐真代码）计数；自定义正则名 SHALL 经集中 `sanitize_kind` 消毒（长度 >32、含 `__`、含 `\x00`、或经大小写归一后不在内置 7 种 + 自定义模式白名单 → 归 `custom_other`，防 label 基数爆炸，`audit_by_rule` 存 `reason` 非 `pattern`；`custom_names` 集合来源为 `PiiDetector.custom_patterns` 各 `(?P<name>...)` 捕获名，实现时由 `PiiDetector` 暴露）；所有计数 SHALL 可按聚合窗口查询且不含明文 PII（仅 `kind` 与 `[REDACTED:<kind>]` 形态，摘要经 `redact_summary(raw,120)` 先脱敏后 `truncate(120)` 单一路径）。**占位符注入不计脱敏计数**：`pii-placeholder-prompt`（`_pii.py:inject_placeholder_prompt`，**实例方法**（`_pii.py:1208` 带 `self`，请求作用域无跨请求状态），请求侧脱敏后、转发前追加 system 说明文本）注入的提示词 SHALL 不计入 `pii_detected_total`/`bytes_in`，且注入前禁止二次 PII 扫描（design D6 实例方法约束，调用方以 `out != in` 判定注入发生与否）；注入发生与否 SHALL 单独计 `placeholder_prompt_injected_total` 并在 `/_admin/health` 暴露 `placeholder_prompt_enabled` 配置。
