...
def _validate_json_roundtrip(original: str, output: str, label: str = "json_walk") -> str:
...
def json_walk(obj, leaf_fn, depth_limit: int = 5, path: str = "$", _depth: int = 0):
        ...
        if inner_stripped.startswith(("{", "[")):
            try:
                ...
                if isinstance(inner, (dict, list)):
                    walked = json_walk(inner, leaf_fn, depth_limit, f"{path}->$.inner", _depth + 1)
    ...
    if isinstance(obj, dict):
        return {k: json_walk(v, leaf_fn, depth_limit, f"{path}.{k}", _depth) for k, v in obj.items()}
    if isinstance(obj, list):
        return [json_walk(x, leaf_fn, depth_limit, f"{path}[{i}]", _depth) for i, x in enumerate(obj)]
    return obj


async def json_walk_async(obj, leaf_fn, depth_limit: int = 5, path: str = "$", _depth: int = 0):
    ...
    if isinstance(obj, str):
        ...
        if inner_stripped.startswith(("{", "[")):
            try:
                inner = _jloads(inner_stripped)
                if isinstance(inner, (dict, list)):
                    walked = await json_walk_async(inner, leaf_fn, depth_limit, f"{path}->$.inner", _depth + 1)
                    return _jdumps(walked)
            except Exception:
                pass
        try:
    ...
    if isinstance(obj, dict):
        ...
        for k, v in obj.items():
            out[k] = await json_walk_async(v, leaf_fn, depth_limit, f"{path}.{k}", _depth)
        return out
    if isinstance(obj, list):
        res = []
        for i, x in enumerate(obj):
            res.append(await json_walk_async(x, leaf_fn, depth_limit, f"{path}[{i}]", _depth))
        return res
    return obj


__all__ = ["json_walk", "json_walk_async", "_strip_bom", "_validate_json_roundtrip", "_jloads", "_jdumps"]
